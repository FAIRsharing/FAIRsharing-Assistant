      ...
      "questions": [
        {
          ...
          "query": {
            ...
            "recordType": ["mock_repository", "mock_knowledgebase_and_repository"],
...
navigationStore.getters = {
  ...
  getPreviousLocation: () => { return "/refine" },
  getRouteQuery: () => {
    return {
      1: {
...
describe("QuestionPage.vue", function(){
  ...
  it("can be instantiated", async() => {
    ...
    await wrapper.vm.$options.watch.foundModelFormats.call(wrapper.vm, foundPoliciesArr)
    await wrapper.vm.$options.watch.foundPolicies.call(wrapper.vm, foundPoliciesArr)
  ...
  it("can check processLink method", async() => {
    ...
    await wrapper.vm.processLink(link, query, message, refined, breadcrumbMod, role, restrict)
  ...
  it("can check processLink method when lowResultsStoppage is true(line 706 in code)", async() => {
    ...
    wrapper.vm.foundPolicies = foundPoliciesArr
    wrapper.vm.lowResultsStoppage = true
    let processLink = await wrapper.vm.processLink(link, query, message, refined, breadcrumbMod, role, restrict)
  ...
  // it("can check processLink method when link does not have 'http'(line 734 in code)", async() => {
  //   wrapper.vm.$route.path = '/1'
  //   wrapper.vm.questionData.questions[0].link = "/1"
  //   let questionObj = wrapper.vm.questionData.questions[0]
  //
  //   let link = questionObj.link
  //   let query = questionObj.query
