      ...
      <v-col cols="12" class="ml-4">
          ...
          >
            ...
            <v-tooltip bottom>
              <template #activator="{ on, attrs }">
                ...
                <div @click="deleteTag(tag.id, tag.model)">
                  <v-icon small class="ml-1" v-on="on" v-bind="attrs">
                    fa-times-circle
                  </v-icon>
                </div>
              </template>
        ...
        >
          ...
          <template #[`item.label`]="{ item }">
            <v-chip :class="colors[item.model] + ' white--text noBreak'">
...
export default {
  ...
  methods: {
    ...
    deleteTag(tagId, tagModel) {
      this.recordTags = this.recordTags.filter(
        (el) => el.id !== tagId && el.model !== tagModel
      );
    },
    goToResults() {
      let _module = this;
